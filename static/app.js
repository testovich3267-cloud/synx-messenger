...
//function openChat(chatId) {
 //   currentChat = chat;
  ...
  //  markAsRead(chat);
//}
async function markAsRead(currentChatId) {
...
function connectWS(chatId) {
    ...
    ws = new WebSocket(`${wsProtocol}//${location.host}/ws/chat/${chatId}?token=${token}`);
    ...
    if (msg.chat_id !== currentChatId) {
        updateChatListUnread(msg.chat_id);
    }
...
function closeMobileSidebar() {
    ...
    if (!mobileSidebarWrapper || !mobileSidebar || !mobileSidebarOverlay) return;

    mobileSidebar.classList.remove("active");
    mobileSidebarOverlay.classList.remove("active");

    setTimeout(() => {
        mobileSidebarWrapper.style.display = "none";
    }, 250);
}
...
async function loadProfile() {
    ...
    const statusSelect = document.querySelector("#statusSelect .synx-select-selected");
if (statusSelect) {
    const map = {
        online: "Онлайн",
        offline: "Оффлайн",
    ...
    statusSelect.textContent = map[me.status] || "Онлайн";
    statusSelect.dataset.value = me.status;
}
...
async function saveProfile() {
    ...
    const selected = document.querySelector("#statusSelect .synx-select-selected");
    const status = selected.dataset.value; // ← фикс
...
fileInput.onchange = () => {
    ...
    fileInput.value = ""; // ← критично для мобильных
};
...
document.querySelectorAll(".synx-select").forEach(select => {
    const selected = select.querySelector(".synx-select-selected");
    const options = select.querySelector(".synx-select-options");
